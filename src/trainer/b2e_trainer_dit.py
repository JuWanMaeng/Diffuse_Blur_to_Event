...
class B2ETrainer_DIT:
    def __init__(
        self,
        cfg: OmegaConf,
        model: B2EPipeline_DIT,
        ...
        self.model: B2EPipeline_DIT = model
        self.device = device
        ...
        self._replace_transformer_proj()
        ...
        self.model.transformer.requires_grad_(True)
    ...
    def _replace_transformer_proj(self):
        # 기존 프로젝션 레이어를 가져옴
        _weight = self.model.transformer.pos_embed.proj.weight.clone()  # [embed_dim, 4, patch_size, patch_size]
        _bias = self.model.transformer.pos_embed.proj.bias.clone()  # [embed_dim]
        ...
        _n_proj_out_channels = self.model.transformer.pos_embed.proj.out_channels  # embed_dim
        _patch_size = self.model.transformer.pos_embed.proj.kernel_size[0]  # 패치 크기 가져오기
        ...
        # 모델의 프로젝션 레이어 교체
        self.model.transformer.pos_embed.proj = _new_proj
        logging.info("transformer proj layer is replaced with 12 channels")

        # 모델 설정(config) 업데이트 (필요한 경우)
        # self.model.transformer.config["in_channels"] = 12
    ...
    def train(self, t_end=None):
        ...
        for epoch in range(self.epoch, self.max_epoch + 1):
            ...
            for batch in skip_first_batches(self.train_loader, self.n_batch_in_epoch):
                ...
                model_pred = self.model.transformer(cat_latents, timesteps,class_labels).sample
