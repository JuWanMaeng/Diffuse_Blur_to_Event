...
if "__main__" == __name__:
    ...
    parser.add_argument(
        "--input_rgb_dir2",
        type=str,
        default='dataset/gopro_t2_part.txt',
        help="Path to the input image folder.",
    )
    parser.add_argument(
        "--dataset_name",
    ...
    input_rgb_dir_2 = args.input_rgb_dir2
    ...
    bi_list = []
    with open(input_rgb_dir_2, 'r') as file:
        for line in file:
            bi_list.append(line.strip())
    bi_list = sorted(bi_list)
    ...
    with torch.no_grad():
        ...
        for idx,rgb_path in enumerate(tqdm(rgb_filename_list, desc="Estimating depth", leave=True)):
            ...
            gt_flow_future_path = rgb_path.replace('blur','flow/flows')

            gt_flow_future_path = gt_flow_future_path.replace('png', 'flo')
            # gt_flow_future_path = gt_flow_future_path.replace('png', 'pfm')
            gt_flow_future = flow_utils.flow_read(gt_flow_future_path)

            nan_mask = np.isnan(gt_flow_future)
            gt_flow_future[nan_mask] = max_flow + 1
            gt_flow_future[nan_mask] = 0
            gt_flow_future = np.clip(gt_flow_future, -max_flow, max_flow)
            gt_flow_future_rgb = flow_to_rgb(gt_flow_future)
            gt_flow_future_rgb = gt_flow_future_rgb[:,:,::-1]

            gt_flow_past_path = bi_list[idx]
            gt_flow_past_path = gt_flow_past_path.replace('blur','flow/flows')

            gt_flow_past_path = gt_flow_past_path.replace('png', 'flo')
            # gt_flow_past_path = gt_flow_past_path.replace('png', 'pfm')
            gt_flow_past = flow_utils.flow_read(gt_flow_past_path)

            nan_mask = np.isnan(gt_flow_past)
            gt_flow_past[nan_mask] = max_flow + 1
            gt_flow_past[nan_mask] = 0
            gt_flow_past = np.clip(gt_flow_past, -max_flow, max_flow)
            gt_flow_past_rgb = flow_to_rgb(gt_flow_past)
            gt_flow_past_rgb = gt_flow_past_rgb[:,:,::-1]
            ...
            gt_future_path = os.path.join(output_dir, img_num, 'gt_future.png')
            gt_past_path = os.path.join(output_dir, img_num, 'gt_past.png')
            ...
            for idx,out_img in enumerate(pipe_out):
                ...
                cv2.imwrite(gt_future_path, gt_flow_future_rgb)
                cv2.imwrite(gt_past_path, gt_flow_past_rgb)
